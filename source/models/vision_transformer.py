...
# Vision transformer recurrent state-space model (RSSM)
class VisionTransformerRSSM(nn.Module):
    """ Vision Transformer """
    def __init__(
        self,
        num_patches,
        ...
        **kwargs
        ...
        self.predictor_norm = norm_layer(predictor_embed_dim)
        self.predictor_proj = nn.Linear(predictor_embed_dim, embed_dim, bias=True)
        self.reward_predictor = nn.Linear(predictor_embed_dim, 1, bias=True)
        # ------
        self.init_std = init_std
        self.apply(self._init_weights)
        self.fix_init_weight()
    ...
    def forward(self, x, poss_x, poss):
        ...
        # -- return preds for mask tokens
        x = x[:, N_ctxt:]
        x1 = self.predictor_proj(x)
        x2 = self.reward_predictor(x)
